...
class ElementParameterSetCommand(CliCommandPort):
    METADATA = CliCommandMetadata(
        id="element_parameter_set",
        logical_component="element",
        description="Set one semantic parameter on one Element instance.",
        arguments=[
            support.argument(
                ["--project"],
                valued=True,
    ...
    def accepts(args: list[str]) -> bool:
        return len(args) == 9 and ElementParameterSetCommand._shape(args[1:])
    ...
    def run(self) -> CommandResponse:
        ...
        return CommandResponse(
            title="Element Parameter Set",
            description="Updated one semantic parameter on one Element instance.",
            content={
